...
An exception: A small number of BioSamples such as [SAMEA968096](https://www.ncbi.nlm.nih.gov/sra/?term=SAMEA968096) are in "sample pools." Such samples will return all run accessions for all BioSamples in that sample pool, often including a generic barcode sample. This may cause issues with downstream analysis; such samples should likely be avoided. There does not appear to be a consistent way to detect sample pools using fasterq-dump, which is why this currently is not handled automatically, but samples within a sample pool are marked on SRA's UI: <img width="382" height="26" alt="Screenshot 2026-08-06 at 3 21 58 PM" src="https://github.com/user-attachments/assets/2622ba59-3040-4492-a442-4638f83fee8f" />

Samples with extremely large FASTQs or samples [such as SAMN17359332 which have a very large number of FASTQs](https://www.ncbi.nlm.nih.gov/sra?LinkName=biosample_sra&from_uid=17359332) can be problematic. Individual files over subsample_cutoff (default 450 MB, -1 disables this check) will be downsampled, but keep an eye on the cumulative size of samples. Because fasterq-dump does not appear able to report the file size of an accession before downloading it, it is possible to run out of disk space before you can even downsample anything.

[I have a compiled denylists of all problematic MTBC samples that I'm aware of](https://github.com/aofarrel/SRANWRP/tree/main/inputs/denylists), including extremely large samples and samples within sample pools, but there may be more out there.
